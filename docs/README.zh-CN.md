...
### 让 🇹🇼 台湾 / 🇸🇬 新加坡 用户看网页版 B 站更顺的 Chrome / Firefox / Edge / Safari 扩充
...
覆盖同名档案（`screenshot-<画面>-<语系>-1280x800.png`）。因为要连真实 bilibili 影片页测速，跑一轮约
数分钟，且吃网络状况。
...
| 🔁 **失败自动切换 CDN** | 默认开启，可在弹出窗口关闭（网络本身不稳、如 WiFi 信号弱时建议关闭，避免频繁黑屏重载）；开启时侦测到分段请求失败（403/404/5xx/网络错误）或播放确实卡住（8 秒内进度不动、且线路上也没有资料在动），先静默切到 B 站原生给的备援节点（分段层即时差替、不整页刷新，并跳出提示）；这个备援也播不动时，改弹出一个不会自动消失的提示，让你自己决定要不要「重载并切换至备用URL」 |
